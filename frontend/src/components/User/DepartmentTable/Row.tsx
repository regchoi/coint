...
const Row: React.FC<RowProps> = ({row, labelId, isItemSelected, handleClick}) => {
    const [departmentinfoModalOpen, setDepartmentinfoModalOpen] = useState(false);

    const handleDepartmentModal = (event: React.MouseEvent<unknown>, id_num: number) => {
        event.stopPropagation();  // 이벤트 전파 중단
        setDepartmentinfoModalOpen(true);
    };
    ...
    return (
        ...
        >
                ...
                if (key === 'detail') {
                    return (
                        <TableCell
                            ...
                            align="center"
                            key={key}
                            onClick={(event) => handleDepartmentModal(event, row.idNum)}
                        >
                            <IconButton>
                                <SearchIcon/>
                            </IconButton>
            ...
            <DepartmentInfoModal open={departmentinfoModalOpen} onClose={() => setDepartmentinfoModalOpen(false)} departmentInfo={row} />
        </TableRow>
    );
};
